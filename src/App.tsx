...
// --- Types ---
type Page = 'beranda' | 'pesan' | 'layanan' | 'akun' | 'login' | 'daftar-mitra' | 'kebijakan' | 'syarat-ketentuan' | 'edit-profil' | 'alamat-saya' | 'iklan-saya' | 'chat' | 'profil-mitra' | 'pesanan' | 'kaffa-cellular' | 'subkategori' | 'peraturan-pelanggan' | 'protokol-mitra';
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="flex items-center px-4 py-4 border-b border-slate-100">
            ...
            <h2 className="ml-4 text-xl font-bold text-slate-800">Lokasi</h2>
          </div>
          ...
          <div className="px-6 py-4">
            <div className="relative flex items-center bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus-within:border-primary/30 transition-all">
              ...
              <input 
                type="text" 
                placeholder="Cari kota, area, atau lingkungan"
                value={search}
          ...
          <div className="flex-1 overflow-y-auto">
              ...
              onClick={() => {
                if (navigator.geolocation) {
                  navigator.geolocation.getCurrentPosition((position) => {
                    ...
                    // For now, we'll just use the currentAddress state.
            ...
            {/* Pilih Negara Bagian (Provinsi) */}
            <div className="px-6 py-6 pb-24">
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Pilih Negara Bagian</h4>
              <button 
  ...
  useEffect(() => {
    ...
    try {
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => {
        ...
        console.warn("Firestore services listener failed (likely permissions):", error);
      });
      return () => unsubscribe();
    } catch (error) {
      console.error("Failed to setup Firestore services listener:", error);
    }
  }, []);
              ...
              <section className="mb-8">
                <div className="relative group">
                  <div className="flex gap-4 overflow-x-auto pb-4 hide-scrollbar -mx-5 px-5 snap-x snap-mandatory">
                    {[
                      { id: 1, name: 'Protokol Keselamatan & Profesionalisme', img: 'https://images.unsplash.com/photo-1581092918056-0c4c3acd3789?w=600', desc: 'Standar Keamanan Mitra', color: 'from-slate-800 to-slate-900', page: 'protokol-mitra', btn: 'Lihat Protokol' },
                      { id: 2, name: 'Solusi Cat', img: 'https://images.unsplash.com/photo-1589939705384-5185137a7f0f?w=600', desc: 'Gratis Konsultasi Warna', color: 'from-emerald-600 to-teal-700', btn: 'Lihat Promo' },
                    ].map((banner) => (
                      <motion.div 
                        key={banner.id}
                        whileTap={{ scale: 0.98 }}
                  ...
                  {[
                    ...
                    >
                      ...
                      <div className="p-3">
                        ...
                        <p className="text-[9px] text-slate-400 font-medium line-clamp-1 mb-3">{mitra.desc}</p>
