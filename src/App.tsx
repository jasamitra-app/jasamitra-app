...
import { collection, query, onSnapshot, addDoc, serverTimestamp, getDocs, where, orderBy, updateDoc, doc, setDoc, getDoc } from 'firebase/firestore';
...
export default function App() {
  ...
  const [mitraStatus, setMitraStatus] = useState<'pending' | 'active' | 'rejected' | null>(null);
          ...
          >
            ...
            <main className="px-5 pt-5 relative z-20 pb-32 bg-slate-50/30">
              ...
              <motion.div 
                ...
                className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex gap-4 mb-3 relative overflow-hidden group cursor-pointer active:scale-95 transition-transform"
              >
              ...
              {/* Protokol Keselamatan Banner - Navy & Orange */}
              <motion.div 
                initial={{ y: 20, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                onClick={() => navigateTo('protokol-mitra')}
                className="bg-primary p-4 rounded-2xl shadow-lg shadow-primary/20 flex gap-4 mb-6 relative overflow-hidden group cursor-pointer active:scale-95 transition-transform border-l-4 border-accent"
              >
                <div className="absolute top-0 right-0 w-32 h-32 bg-accent/20 rounded-full -mr-12 -mt-12 blur-3xl opacity-50" />
                <div className="w-10 h-10 bg-accent rounded-xl flex items-center justify-center text-white shrink-0 relative z-10 shadow-lg shadow-accent/30">
                  <ShieldCheck size={20} />
                </div>
                <div className="relative z-10">
                  <h4 className="text-[10px] font-black text-white uppercase tracking-wider mb-0.5 flex items-center gap-1.5">
                    Protokol Keselamatan & Profesionalisme
                  </h4>
                  <p className="text-[8px] text-white/80 leading-relaxed font-medium">
                    Pedoman kerja mitra untuk layanan aman & terpercaya.
                  </p>
                </div>
                <div className="ml-auto self-center text-accent">
